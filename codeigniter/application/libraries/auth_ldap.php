...
class auth_ldap {
    ...
    private function _authenticate($username, $password) {
        $needed_attrs = array('dn', 'cn', $this->login_attribute, 'displayName', 'mail');
	    ...
	    $mail = $entries[0]['mail'][0];
            ...
            $filter = '('.$this->login_attribute.'='.$username.')';
            $search = ldap_search($this->ldapconn, $this->basedn, $filter, array('dn', $this->login_attribute, 'cn', 'displayName', 'mail'));
            $entries = ldap_get_entries($this->ldapconn, $search);
            $binddn = $entries[0]['dn'];
        ...
        return array('cn' => $cn, 'dn' => $dn, 'id' => $id,
            'role' => $this->_get_role($get_role_arg), 'displayName' => $displayName, 'mail' => $mail);
    }
